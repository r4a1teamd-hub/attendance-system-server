...
def update_student_data():
    with app.app_context():
        student_id = "20240001"
        email = "nitikaryuya@gmail.com"
        
        user = User.query.filter_by(student_id=student_id).first()
        
        if not user:
            print(f"User {student_id} not found. Creating...")
            user = User(student_id=student_id, username="Student 20240001", email=email, role=0)
        ...
        # Add 200 absent records
        print(f"Adding 200 absent records for {student_id}...")
